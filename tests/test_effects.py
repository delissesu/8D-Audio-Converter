...
def apply_panning(samples, sr, pan_speed=0.15, pan_depth=1.0):
    effect = Rotate8DEffect()
    params = {"pan_speed": pan_speed, "pan_depth": pan_depth}
    return effect.apply(samples, sr, params)


def apply_reverb(samples, sr, room_size=0.4, wet_level=0.3, damping=0.5):
    effect = ReverbEffect()
    params = {"room_size": room_size, "wet_level": wet_level, "damping": damping}
    return effect.apply(samples, sr, params)
...
class TestNormalizeAudio:
    ...
    def test_peak_is_near_099(self) -> None:
        samples: np.ndarray = make_stereo_sine() * 5.0
...
class TestValidateParamRange:
    ...
    def test_valid_range_passes(self) -> None:
        validate_param_range(0.5, "test", 0.0, 1.0)
    ...
    def test_at_exact_minimum_passes(self) -> None:
        validate_param_range(0.0, "pan_depth", 0.0, 1.0)

    def test_at_exact_maximum_passes(self) -> None:
        validate_param_range(1.0, "pan_depth", 0.0, 1.0)
...
class TestConvertTo8D:
    ...
    def test_output_is_stereo(self, tmp_path: str) -> None:
        ...
        convert_to_8d(in_path, out_path)
    ...
    def test_missing_input_raises_file_not_found(self, tmp_path: str) -> None:
        ...
        with pytest.raises(FileNotFoundError):
            convert_to_8d("nonexistent.wav", out_path)

    def test_invalid_output_extension_raises(self, tmp_path: str) -> None:
        in_path: str = os.path.join(str(tmp_path), "input.wav")
        make_test_wav(in_path)
        with pytest.raises(ValueError, match="Unsupported output format"):
            convert_to_8d(in_path, "output.txt")
    ...
    def test_pan_speed_out_of_range_raises(self, tmp_path: str) -> None:
        ...
        make_test_wav(in_path)
        with pytest.raises(ValueError, match="pan_speed"):
            convert_to_8d(in_path, out_path, pan_speed=99.0)
    ...
    def test_room_size_out_of_range_raises(self, tmp_path: str) -> None:
        ...
        with pytest.raises(ValueError, match="room_size"):
            convert_to_8d(in_path, out_path, room_size=1.5)
    ...
    def test_custom_params_produce_different_output(self, tmp_path: str) -> None:
        ...
        convert_to_8d(in_path, out_a, pan_speed=0.1, room_size=0.2)
        convert_to_8d(in_path, out_b, pan_speed=0.8, room_size=0.9)
